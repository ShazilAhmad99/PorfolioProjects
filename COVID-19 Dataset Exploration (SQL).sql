-- In this project, we will be looking at many COVID-19 statistics from countires across the world, with a focus on the country of Japan. The data was provided by the paper "Coronavirus (COVID-19) Deaths" bey Mathieu et al. 
...
ORDER BY 1 
;
-- Here we can see that the case to population percentage first reached the maximum amount oof 27.27% on May 14, 2023. 
...
-- Next we can look at the death rates for the categories other than location. 
...
-- Now that we have explored the "deaths" table, let us also join it with the other "vaccines" table and explore the resulting table. 
...
-- Let's do the same with all countries in the dataset, replacing new_vaccinations.
SELECT deaths.location, deaths.date, deaths.population, new_vaccinations, 
